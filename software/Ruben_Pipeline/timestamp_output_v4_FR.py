...
ENHANCEMENTS:
- Added force-target-count mode that guarantees detecting the exact number of repetitions specified
- Progressive parameter relaxation system to adapt to challenging signals
- Quality indicators for detected repetitions
- Enhanced visualization for forced detections
- Comprehensive logging for diagnostic purposes

Usage:
python enhanced_deadlift_detector.py input_file.csv --target-rep-count 6 --export-format both --force-target-count
...
class ForcedRepCountProcessor:
    """
    Implements the progressive parameter relaxation system to guarantee detecting
    the exact number of repetitions specified in target_rep_count.
    """
    def __init__(self, detector, target_count, verbose=False):
        """
        Initialize the forced repetition count processor.
        
        Args:
            detector: Reference to the main EnhancedDeadliftDetector instance
            target_count: Target number of repetitions to detect
    ...
    def ensure_target_rep_count(self, current_phases):
        """
        Ensure the exact number of repetitions is detected.
        
        Args:
            current_phases: List of already detected phases
            
        Returns:
            List of exactly target_count phases
        """
        if len(current_phases) == self.target_count:
            # Already have the target count, nothing to do
            return current_phases
        
        if len(current_phases) > self.target_count:
            # Too many phases, select the best ones
            return self._select_best_phases(current_phases)
        
        if self.verbose:
            print(f"\n--- Force Target Count mode: Detected {len(current_phases)}/{self.target_count} reps " +
                 f"with standard detection, activating progressive relaxation ---")
        
        # Store original parameters before modification
        self._store_original_parameters()
        
        # Initialize parameters for the relaxation process
        phases = current_phases.copy()
        phases_by_step = {
            "original": phases.copy()
        }
        
        # Phase 1: Parameter Relaxation (5 steps)
        if len(phases) < self.target_count:
            phases = self._apply_parameter_relaxation(phases)
            
        # Phase 2: Signal Re-analysis with alternative processing methods
        if len(phases) < self.target_count:
            additional_phases = self._apply_signal_reanalysis()
            phases = self._merge_phases(phases, additional_phases)
            
        # Phase 3: Peak-Based Inference
        if len(phases) < self.target_count:
            additional_phases = self._apply_peak_based_inference()
            phases = self._merge_phases(phases, additional_phases)
        
        # Phase 4: Time-Based Division (as last resort)
        if len(phases) < self.target_count:
            additional_phases = self._apply_time_based_division()
            phases = self._merge_phases(phases, additional_phases)
            
        # Final selection and quality scoring
        phases = self._select_best_phases(phases)
        phases = self._assign_quality_scores(phases)
        
        # Restore original parameters
        self._restore_original_parameters()
        
        if self.verbose:
            print(f"\n--- Force Target Count mode: Successfully ensured {len(phases)}/{self.target_count} repetitions ---")
            quality_counts = {}
            for phase in phases:
                quality = phase.get("quality", "unknown")
                quality_counts[quality] = quality_counts.get(quality, 0) + 1
            
            for quality, count in quality_counts.items():
                print(f"  {quality.capitalize()} quality: {count} repetitions")
        
        return phases
    
    def _store_original_parameters(self):
        """Store original detection parameters before modification."""
        self.original_params = {
            "peak_threshold": self.detector.peak_threshold,
            "baseline_threshold": self.detector.baseline_threshold,
            "min_duration": self.detector.min_duration,
            "max_duration": self.detector.max_duration,
            "selected_column": self.detector.selected_column,
            "should_negate": self.detector.should_negate
        }
        
    def _restore_original_parameters(self):
        """Restore original detection parameters."""
        if hasattr(self, "original_params"):
            self.detector.peak_threshold = self.original_params["peak_threshold"]
            self.detector.baseline_threshold = self.original_params["baseline_threshold"]
            self.detector.min_duration = self.original_params["min_duration"]
            self.detector.max_duration = self.original_params["max_duration"]
            self.detector.selected_column = self.original_params["selected_column"]
            self.detector.should_negate = self.original_params["should_negate"]
    
    def _apply_parameter_relaxation(self, current_phases):
        """
        Apply progressive parameter relaxation to find more repetitions.
        
        Args:
            current_phases: Currently detected phases
            
        Returns:
            Updated list of phases after relaxation
        """
        if self.verbose:
            print("\nPhase 1: Parameter Relaxation")
        
        # Start with current phases
        phases = current_phases.copy()
        
        # Step 1: Reduce minimum duration by 50%
        if len(phases) < self.target_count:
            if self.verbose:
                print(f"  Step 1: Reducing minimum duration by 50% (from {self.detector.min_duration:.2f}s)")
            
            original_min_duration = self.detector.min_duration
            self.detector.min_duration *= 0.5
            self.relaxation_steps.append("min_duration_reduced")
            
            # Try to detect more phases with the new parameters
            additional_phases = self._detect_additional_phases(phases)
            if additional_phases:
                for phase in additional_phases:
                    phase["relaxation_step"] = "min_duration_reduced"
                    phase["quality"] = "medium"
                
                phases = self._merge_phases(phases, additional_phases)
                
                if self.verbose:
                    print(f"    Found {len(additional_phases)} additional phases, now have {len(phases)}/{self.target_count}")
            
            # If we found enough phases, return early
            if len(phases) >= self.target_count:
                return self._select_best_phases(phases)
        
        # Step 2: Reduce peak threshold by 40%
        if len(phases) < self.target_count:
            if self.verbose:
                print(f"  Step 2: Reducing peak threshold by 40% (from {self.detector.peak_threshold:.4f}g)")
            
            original_peak_threshold = self.detector.peak_threshold
            self.detector.peak_threshold *= 0.6  # 40% reduction
            self.relaxation_steps.append("peak_threshold_reduced")
            
            # Try to detect more phases with the new parameters
            additional_phases = self._detect_additional_phases(phases)
            if additional_phases:
                for phase in additional_phases:
                    phase["relaxation_step"] = "peak_threshold_reduced"
                    phase["quality"] = "medium"
        ...
        # Step 3: Reduce baseline threshold by 60%
        if len(phases) < self.target_count:
            if self.verbose:
                print(f"  Step 3: Reducing baseline threshold by 60% (from {self.detector.baseline_threshold:.4f}g)")
            
            original_baseline_threshold = self.detector.baseline_threshold
            self.detector.baseline_threshold *= 0.4  # 60% reduction
            self.relaxation_steps.append("baseline_threshold_reduced")
            
            # Try to detect more phases with the new parameters
            additional_phases = self._detect_additional_phases(phases)
            if additional_phases:
                for phase in additional_phases:
                    phase["relaxation_step"] = "baseline_threshold_reduced"
                    phase["quality"] = "medium"
        ...
        # Step 4: Completely relax all thresholds
        if len(phases) < self.target_count:
            if self.verbose:
                print(f"  Step 4: Drastically relaxing all thresholds and constraints")
            
            # Save original values
            original_peak_threshold = self.detector.peak_threshold
            original_baseline_threshold = self.detector.baseline_threshold
            original_min_duration = self.detector.min_duration
            original_max_duration = self.detector.max_duration
            
            # Apply extreme relaxation
            self.detector.peak_threshold *= 0.3  # 70% further reduction
            self.detector.baseline_threshold *= 0.3  # 70% further reduction
            self.detector.min_duration *= 0.5  # Further 50% reduction
            self.detector.max_duration *= 1.5  # 50% increase
            self.relaxation_steps.append("extreme_relaxation")
            
            # Try to detect more phases with the new parameters
            additional_phases = self._detect_additional_phases(phases)
            if additional_phases:
                for phase in additional_phases:
                    phase["relaxation_step"] = "extreme_relaxation"
                    phase["quality"] = "low"
        ...
        # Step 5: Disable validation checks
        if len(phases) < self.target_count:
            if self.verbose:
                print(f"  Step 5: Disabling validation checks to find all potential repetitions")
            
            # Detect potential repetitions without validation
            self.relaxation_steps.append("validation_disabled")
            additional_phases = self._detect_without_validation(phases)
            
            if additional_phases:
                for phase in additional_phases:
                    phase["relaxation_step"] = "validation_disabled"
                    phase["quality"] = "low"
    ...
    def _apply_signal_reanalysis(self):
        """
        Re-analyze the signal with alternative approaches.
        
        Returns:
            List of new phases found through re-analysis
        """
        if self.verbose:
            print("\nPhase 2: Signal Re-analysis")
        
        new_phases = []
        
        # 1. Try alternative orientation
        if self.verbose:
            print("  Trying alternative orientation...")
        
        original_should_negate = self.detector.should_negate
        self.detector.should_negate = not original_should_negate
        
        # Reprocess signal with new orientation
        processed_data = self.detector.signal_processor.process_signal(
            self.detector.raw_data, 
            self.detector.selected_column, 
            self.detector.profile, 
            self.detector.should_negate
        )
        
        # Find peaks and phases with new orientation
        peaks = self.detector._find_peaks(processed_data)
        
        orientation_phases = []
        for peak in peaks:
            phase = self.detector._find_concentric_phase(processed_data, peak["index"])
            if phase:
                phase["reanalysis_method"] = "alternative_orientation"
                phase["quality"] = "medium"
                orientation_phases.append(phase)
        
        # Filter and add to new phases
        if orientation_phases:
            orientation_phases = self.detector._filter_overlapping_phases(orientation_phases)
            new_phases.extend(orientation_phases)
            
            if self.verbose:
                print(f"    Found {len(orientation_phases)} phases with alternative orientation")
        
        # Restore original orientation
        self.detector.should_negate = original_should_negate
        
        # 2. Try alternative acceleration columns
        accel_cols = self.detector.profile["columns"]["acceleration"] + self.detector.profile["columns"]["vertical_acceleration"]
        
        # Filter out the already used column
        accel_cols = [col for col in accel_cols if col != self.detector.selected_column and col in self.detector.raw_data.columns]
        
        if accel_cols and self.verbose:
            print(f"  Trying {len(accel_cols)} alternative acceleration columns...")
        
        for col in accel_cols:
            # Analyze this column in both orientations
            for negate in [False, True]:
                # Process the signal
                processed_data = self.detector.signal_processor.process_signal(
                    self.detector.raw_data, col, self.detector.profile, negate
                )
                
                # Find peaks and phases
                peaks = self.detector._find_peaks(processed_data)
                ...
                for peak in peaks:
                    phase = self.detector._find_concentric_phase(processed_data, peak["index"])
                    if phase:
                        phase["reanalysis_method"] = f"alternative_column_{col}_{'negated' if negate else 'original'}"
                        phase["quality"] = "medium"
                        alt_col_phases.append(phase)
                
                # Add to new phases if we found any
                if alt_col_phases:
                    alt_col_phases = self.detector._filter_overlapping_phases(alt_col_phases)
                    new_phases.extend(alt_col_phases)
                    
                    if self.verbose:
                        print(f"    Found {len(alt_col_phases)} phases from column {col} ({'negated' if negate else 'original'})")
        
        # 3. Try alternative filtering methods
        if self.verbose:
            print("  Trying alternative filtering methods...")
        
        # Get current filter type
        current_filter = self.detector.profile["recommended_params"].get("filter_type", "butterworth")
        
        # Try other filter types
        alt_filters = {
            "butterworth": {"cutoff_freq": 3.0, "filter_order": 2},
            "savgol": {"window_length": 11, "poly_order": 3},
            "moving_avg": {"window_size": 9}
        }
        
        # Remove the current filter from the alternatives
        if current_filter in alt_filters:
            del alt_filters[current_filter]
        
        for filter_name, params in alt_filters.items():
            processor = AdaptiveSignalProcessor(verbose=False)
            
            # Process with this filter
            if filter_name == "butterworth":
                processed_data = processor._preprocess_signal(self.detector.data_processed)
                processed_data = processor._apply_butterworth_filter(
                    processed_data, 
                ...
                processed_data = processor._preprocess_signal(self.detector.data_processed)
                processed_data = processor._apply_savgol_filter(
                    processed_data,
                ...
                processed_data = processor._preprocess_signal(self.detector.data_processed)
                processed_data = processor._apply_moving_average(
                    processed_data,
                    window_size=params["window_size"]
                )
            
            # Find peaks and phases with this filter
            peaks = self.detector._find_peaks_internal(
                processed_data, 
                self.detector.peak_threshold,
                self.detector.baseline_threshold,
                self.detector.min_duration,
                processor.sampling_rate
            )
            
            filt_phases = []
            for peak in peaks:
                phase = self.detector._find_concentric_phase_internal(
                    processed_data, 
                    peak["index"],
                    self.detector.peak_threshold,
                    self.detector.baseline_threshold,
                    self.detector.min_duration,
                    self.detector.max_duration,
                    processor.sampling_rate
                )
                if phase:
                    phase["reanalysis_method"] = f"alternative_filter_{filter_name}"
                    phase["quality"] = "medium"
                    filt_phases.append(phase)
            
            # Add to new phases if we found any
            if filt_phases:
                filt_phases = self.detector._filter_overlapping_phases(filt_phases)
                new_phases.extend(filt_phases)
                
                if self.verbose:
                    print(f"    Found {len(filt_phases)} phases with {filter_name} filter")
        
        # Remove overlapping phases from the combined results
        if new_phases:
            new_phases = self.detector._filter_overlapping_phases(new_phases)
            
            if self.verbose:
                print(f"  Total phases found through re-analysis: {len(new_phases)}")
        
        return new_phases
    
    def _apply_peak_based_inference(self):
        """
        Use peak detection and direct inference to create phases.
        
        Returns:
            List of phases created through peak-based inference
        """
        if self.verbose:
            print("\nPhase 3: Peak-Based Inference")
        
        # Try to find all potential peaks, even small ones
        original_peak_threshold = self.detector.peak_threshold
        self.detector.peak_threshold *= 0.2  # Very low threshold to find more potential peaks
        
        peaks = self.detector._find_peaks(self.detector.data_processed)
        
        # Restore original threshold
        self.detector.peak_threshold = original_peak_threshold
        
        if not peaks:
            if self.verbose:
                print("  No peaks found for inference")
            return []
        
        # Get information about the successfully detected phases
        existing_phases = self.detector.phases
        existing_peaks = [phase["peakIdx"] for phase in existing_phases]
        
        # Filter out peaks that already correspond to detected phases
        new_peaks = []
        for peak in peaks:
            if peak["index"] not in existing_peaks:
                new_peaks.append(peak)
        
        if self.verbose:
            print(f"  Found {len(new_peaks)} potential new peaks for inference")
        
        if not new_peaks:
            return []
        
        # Sort peaks by value (highest first) to get the strongest peaks
        sorted_peaks = sorted(new_peaks, key=lambda x: x["value"], reverse=True)
        
        # Determine how many more phases we need
        needed_phases = self.target_count - len(existing_phases)
        peaks_to_use = sorted_peaks[:min(len(sorted_peaks), needed_phases * 2)]  # Get more than needed for filtering
        
        # Create phases from these peaks with estimated durations
        inferred_phases = []
        
        # Calculate average duration from existing phases to use as reference
        avg_duration = 0.5  # Default duration in seconds
        if existing_phases:
            existing_durations = [phase["duration"] for phase in existing_phases]
            if existing_durations:
                avg_duration = np.mean(existing_durations)
        
        sampling_rate = self.detector.signal_processor.sampling_rate
        avg_duration_samples = int(avg_duration * sampling_rate)
        
        # Create phases from peaks
        for i, peak in enumerate(peaks_to_use):
            peak_idx = peak["index"]
            peak_time = peak["time"]
            peak_value = peak["value"]
            
            # Use half of average duration before peak, half after
            half_duration_samples = max(1, avg_duration_samples // 2)
            start_idx = max(0, peak_idx - half_duration_samples)
            end_idx = min(len(self.detector.data_processed) - 1, peak_idx + half_duration_samples)
            
            start_time = self.detector.data_processed["time_s"].iloc[start_idx]
            end_time = self.detector.data_processed["time_s"].iloc[end_idx]
            duration = end_time - start_time
            
            # Create inferred phase
            phase = {
                ...
                "peakIdx": peak_idx,
                "isInferred": True,
                "inferenceMethod": "peak_based",
                "quality": "low"
            }
            
            inferred_phases.append(phase)
        
        # Filter out overlapping phases
        if inferred_phases:
            inferred_phases = self.detector._filter_overlapping_phases(inferred_phases)
        
        if self.verbose:
            print(f"  Created {len(inferred_phases)} phases through peak-based inference")
        
        return inferred_phases
        
    def _apply_time_based_division(self):
        """
        Divide the signal into regular time segments and create synthetic phases.
        This is the last resort method when all else fails.
        
        Returns:
            List of synthetically created phases
        """
        if self.verbose:
            print("\nPhase 4: Time-Based Division (Last Resort)")
        
        # Get information about the existing phases
        existing_phases = self.detector.phases
        ...
        # Get data timing information
        data = self.detector.data_processed
        
        start_time = data["time_s"].iloc[0]
        end_time = data["time_s"].iloc[-1]
        total_duration = end_time - start_time
        
        # Calculate average duration from existing phases if available
        avg_duration = 0.5  # Default duration in seconds
        if existing_phases:
            existing_durations = [phase["duration"] for phase in existing_phases]
            if existing_durations:
        ...
        # Calculate average rest time between reps if available
        avg_rest_time = 1.0  # Default rest time in seconds
        if len(existing_phases) > 1:
            rest_times = []
            sorted_phases = sorted(existing_phases, key=lambda x: x["startTime"])
            for i in range(1, len(sorted_phases)):
                rest_time = sorted_phases[i]["startTime"] - sorted_phases[i-1]["endTime"]
                rest_times.append(rest_time)
            
            if rest_times:
                avg_rest_time = np.mean(rest_times)
        
        # Calculate cycle time (rep + rest)
        cycle_time = avg_duration + avg_rest_time
        
        # Create synthetic repetitions at regular intervals
        synthetic_phases = []
        
        # If we have existing phases, try to fill in the gaps
        if existing_phases:
            # Sort phases by start time
            sorted_phases = sorted(existing_phases, key=lambda x: x["startTime"])
            
            # Try to estimate missing phases in the correct places
            # First, look for large gaps between existing phases
            gaps = []
            
            # Check the gap from the start of the data to the first phase
            if sorted_phases[0]["startTime"] - start_time > cycle_time * 1.5:
                gaps.append({
                    "start_time": start_time,
                    "end_time": sorted_phases[0]["startTime"],
                    "duration": sorted_phases[0]["startTime"] - start_time,
                    "possible_reps": int((sorted_phases[0]["startTime"] - start_time) / cycle_time)
                })
            
            # Check gaps between phases
            for i in range(1, len(sorted_phases)):
                gap_start = sorted_phases[i-1]["endTime"]
                gap_end = sorted_phases[i]["startTime"]
                gap_duration = gap_end - gap_start
                
                if gap_duration > cycle_time * 1.5:
                    gaps.append({
                        "start_time": gap_start,
                        "end_time": gap_end,
                        "duration": gap_duration,
                        "possible_reps": int(gap_duration / cycle_time)
                    })
            
            # Check the gap from the last phase to the end of the data
            if end_time - sorted_phases[-1]["endTime"] > cycle_time * 1.5:
                gaps.append({
                    "start_time": sorted_phases[-1]["endTime"],
                    "end_time": end_time,
                    "duration": end_time - sorted_phases[-1]["endTime"],
                    "possible_reps": int((end_time - sorted_phases[-1]["endTime"]) / cycle_time)
                })
            
            # Fill in the gaps with synthetic phases
            if gaps:
                if self.verbose:
                    print(f"  Found {len(gaps)} significant gaps between existing repetitions")
                
                # Prioritize gaps by number of possible reps
                gaps.sort(key=lambda x: x["possible_reps"], reverse=True)
                
                # Create phases in the gaps
                reps_created = 0
                for gap in gaps:
                    if reps_created >= needed_phases:
                        break
                    
                    possible_reps = min(gap["possible_reps"], needed_phases - reps_created)
                    
                    if possible_reps <= 0:
                        continue
                    
                    # Create evenly spaced phases within this gap
                    gap_duration = gap["duration"]
                    spacing = gap_duration / (possible_reps + 1)
                    
                    for i in range(possible_reps):
                        phase_start_time = gap["start_time"] + spacing * (i + 1) - avg_duration / 2
                        phase_end_time = phase_start_time + avg_duration
                        phase_peak_time = phase_start_time + avg_duration * 0.3  # Peak at 30% of phase
                        
                        # Find indices corresponding to these times
                        start_idx = self._find_closest_time_index(data, phase_start_time)
                        end_idx = self._find_closest_time_index(data, phase_end_time)
                        peak_idx = self._find_closest_time_index(data, phase_peak_time)
                        
                        # Find the maximum acceleration value within this region for a realistic peak
                        phase_data = data["smoothed"].iloc[start_idx:end_idx+1]
                        if len(phase_data) > 0:
                            peak_value = phase_data.max()
                            peak_idx = start_idx + phase_data.argmax()
                        else:
                            # Fallback if no data points in region
                            peak_value = 0.1  # Minimal value
                        
                        # Create the synthetic phase
                        phase = {
                            "startTime": phase_start_time,
                            "endTime": phase_end_time,
                            "peakTime": data["time_s"].iloc[peak_idx],
                            ...
                            "peakIdx": peak_idx,
                            "isSynthetic": True,
                            "synthMethod": "gap_filling",
                            "quality": "synthetic"
                        }
                        
                        synthetic_phases.append(phase)
                        reps_created += 1
            
            # If we still need more phases, use time-based division for the remainder
            if len(synthetic_phases) < needed_phases:
                if self.verbose:
                    print(f"  Still need {needed_phases - len(synthetic_phases)} more repetitions")
                
                # Create additional synthetic phases by looking at overall signal energy
                remaining_needed = needed_phases - len(synthetic_phases)
                
                # Look for regions with higher signal energy that haven't been used yet
                # Get all existing and synthetic phase regions
                all_phases = existing_phases + synthetic_phases
                all_phase_ranges = [(p["startIdx"], p["endIdx"]) for p in all_phases]
                
                # Create a mask of unused regions
                used_indices = np.zeros(len(data), dtype=bool)
                for start_idx, end_idx in all_phase_ranges:
                    if start_idx < end_idx and end_idx < len(used_indices):
                        used_indices[start_idx:end_idx+1] = True
                
                # Calculate energy in the signal for unused regions
                energy = data["smoothed"].values ** 2
                energy[used_indices] = 0  # Zero out used regions
                
                # Use a sliding window to find regions with high energy
                window_size = int(avg_duration * self.detector.signal_processor.sampling_rate)
                window_size = max(10, window_size)  # Ensure minimum window size
                
                energy_windows = []
                
                for i in range(0, len(energy) - window_size, window_size // 2):
                    window_energy = np.sum(energy[i:i+window_size])
                    if window_energy > 0:
                ...
                # Sort windows by energy (highest first)
                energy_windows.sort(key=lambda x: x["energy"], reverse=True)
                
                # Create phases from the highest energy windows
                for i, window in enumerate(energy_windows[:remaining_needed]):
                    start_idx = window["start_idx"]
                    end_idx = window["end_idx"]
                    
                    # Find the peak within this window
                    window_data = data["smoothed"].iloc[start_idx:end_idx+1]
                    if len(window_data) > 0:
                        peak_value = window_data.max()
                        peak_idx = start_idx + window_data.argmax()
                    else:
                        # Fallback
                        peak_idx = (start_idx + end_idx) // 2
                        peak_value = 0.1
                    
                    # Create the synthetic phase
                    phase = {
                        "startTime": data["time_s"].iloc[start_idx],
                        "endTime": data["time_s"].iloc[end_idx],
                        "peakTime": data["time_s"].iloc[peak_idx],
                        "duration": data["time_s"].iloc[end_idx] - data["time_s"].iloc[start_idx],
                        "peakAcceleration": peak_value,
                        "startIdx": start_idx,
                        "endIdx": end_idx,
                        "peakIdx": peak_idx,
                        "isSynthetic": True,
                        "synthMethod": "energy_based",
                        "quality": "synthetic"
                    }
        ...
        # If we still need more phases or had no existing phases,
        # distribute them evenly across the entire time range
        if len(synthetic_phases) < needed_phases:
            remaining_needed = needed_phases - len(synthetic_phases)
            
            # Create a set of times where we already have phases to avoid overlap
            all_phases = existing_phases + synthetic_phases
            
            # Sort by start time
            all_phases.sort(key=lambda x: x["startTime"])
            
            # Find available regions
            available_regions = []
            
            # First region: from start of data to first phase
            if not all_phases:
                # No phases yet, use entire range
                ...
                # Check if there's space before the first phase
                if all_phases[0]["startTime"] - start_time > avg_duration:
                    available_regions.append({
                        "start_time": start_time,
                        "end_time": all_phases[0]["startTime"] - avg_rest_time,
                        "duration": all_phases[0]["startTime"] - avg_rest_time - start_time
                    })
                
                # Check spaces between phases
                for i in range(1, len(all_phases)):
                    gap_start = all_phases[i-1]["endTime"] + avg_rest_time
                    gap_end = all_phases[i]["startTime"] - avg_rest_time
                ...
                # Check if there's space after the last phase
                if end_time - all_phases[-1]["endTime"] > avg_duration:
                    available_regions.append({
                        "start_time": all_phases[-1]["endTime"] + avg_rest_time,
                        "end_time": end_time,
                        "duration": end_time - (all_phases[-1]["endTime"] + avg_rest_time)
                    })
            
            # Sort regions by duration (largest first)
            available_regions.sort(key=lambda x: x["duration"], reverse=True)
            
            # Create phases in the largest regions first
            reps_created = 0
            for region in available_regions:
                if reps_created >= remaining_needed:
                    break
                
                # Calculate how many reps can fit in this region
                possible_reps = max(1, int(region["duration"] / (avg_duration + avg_rest_time)))
                reps_to_create = min(possible_reps, remaining_needed - reps_created)
                
                if reps_to_create <= 0:
                    continue
                
                # Create evenly spaced phases within this region
                spacing = region["duration"] / (reps_to_create + 1)
                
                for i in range(reps_to_create):
                    phase_center = region["start_time"] + spacing * (i + 1)
                    phase_start_time = phase_center - avg_duration / 2
                    phase_end_time = phase_center + avg_duration / 2
                    phase_peak_time = phase_start_time + avg_duration * 0.3  # Peak at 30% of phase
                    ...
                    phase = {
                        "startTime": phase_start_time,
                        "endTime": phase_end_time,
                        "peakTime": phase_peak_time,
                        "duration": phase_end_time - phase_start_time,
                        "peakAcceleration": 0.1,  # Minimal value
                        "startIdx": start_idx,
                        "endIdx": end_idx,
                        "peakIdx": peak_idx,
                        "isSynthetic": True,
                        "synthMethod": "uniform_distribution",
                        "quality": "synthetic"
                    }
        ...
        if self.verbose:
            print(f"  Created {len(synthetic_phases)} synthetic phases")
        
        return synthetic_phases
    
    def _find_closest_time_index(self, data, target_time):
        """Find the index in the data that has the closest time to the target time."""
        time_diffs = abs(data["time_s"] - target_time)
        return time_diffs.argmin()
    
    def _detect_additional_phases(self, existing_phases):
        """
        Detect additional phases using current parameters but excluding existing ones.
        
        Args:
            existing_phases: List of already detected phases
            
        Returns:
            List of newly detected phases
        """
        # Get peaks with current parameters
        peaks = self.detector._find_peaks(self.detector.data_processed)
        
        if not peaks:
            return []
        
        # Get the indices of peaks that are already part of existing phases
        existing_peak_indices = set(phase["peakIdx"] for phase in existing_phases)
        
        # Filter out peaks that already correspond to phases
        new_peaks = [peak for peak in peaks if peak["index"] not in existing_peak_indices]
        
        if not new_peaks:
            return []
        
        # Detect phases for these new peaks
        new_phases = []
        for peak in new_peaks:
            phase = self.detector._find_concentric_phase(self.detector.data_processed, peak["index"])
            if phase:
                new_phases.append(phase)
        
        # Filter overlapping phases
        if new_phases:
            # First filter out phases that overlap with existing phases
            non_overlapping_phases = []
            for new_phase in new_phases:
                overlaps_existing = False
                for existing_phase in existing_phases:
                    # Check for overlap
                    overlap_start = max(new_phase["startTime"], existing_phase["startTime"])
                    overlap_end = min(new_phase["endTime"], existing_phase["endTime"])
                    
                    if overlap_end > overlap_start:
                        # Calculate overlap percentage
                        new_duration = new_phase["endTime"] - new_phase["startTime"]
                        overlap_duration = overlap_end - overlap_start
                        
                        overlap_percentage = overlap_duration / new_duration
                        
                        if overlap_percentage > 0.25:  # More than 25% overlap
            ...
            # Then filter out phases that overlap with each other
            filtered_phases = self.detector._filter_overlapping_phases(non_overlapping_phases)
            
            return filtered_phases
        
        return []
    
    def _detect_without_validation(self, existing_phases):
        """
        Detect potential repetitions without applying validation checks.
        ...
        Returns:
            List of newly detected phases
        """
        # Try to find all potential peaks
        processed_data = self.detector.data_processed
        
        # Find all local maxima in the signal without filtering
        smoothed = processed_data["smoothed"].values
        potential_peaks = []
        ...
        for i in range(2, len(smoothed) - 2):
            if (smoothed[i] > smoothed[i-1] and 
                ...
                smoothed[i] > smoothed[i+2] and
                smoothed[i] > self.detector.baseline_threshold):
                
                potential_peaks.append({
                    "index": i,
                    "time": processed_data["time_s"].iloc[i],
                    "value": smoothed[i]
                })
        ...
        # Remove peaks that are already in existing phases
        existing_peak_times = [phase["peakTime"] for phase in existing_phases]
        
        new_peaks = []
        for peak in potential_peaks:
            # Check if this peak is close to an existing peak
            is_new = True
            for existing_time in existing_peak_times:
                if abs(peak["time"] - existing_time) < 0.2:  # Within 0.2 seconds
                    is_new = False
                    break
        ...
        new_peaks.sort(key=lambda x: x["value"], reverse=True)
        
        # Create simplified phases around these peaks
        sampling_rate = self.detector.signal_processor.sampling_rate
        avg_duration = 0.5  # Default duration in seconds
        
        if existing_phases:
            durations = [phase["duration"] for phase in existing_phases]
            if durations:
                avg_duration = np.mean(durations)
        
        half_duration_samples = int(avg_duration * sampling_rate / 2)
        ...
        for peak in new_peaks:
            ...
            # Create a phase around this peak
            start_idx = max(0, peak_idx - half_duration_samples)
            end_idx = min(len(processed_data) - 1, peak_idx + half_duration_samples)
        ...
        # Filter out phases that overlap with existing ones
        non_overlapping_phases = []
        for new_phase in new_phases:
            overlaps_existing = False
            for existing_phase in existing_phases:
        ...
        # Also filter overlaps between new phases
        filtered_phases = self.detector._filter_overlapping_phases(non_overlapping_phases)
        
        return filtered_phases
    
    def _merge_phases(self, existing_phases, new_phases):
        """
        Merge existing phases with new phases, handling overlaps.
        
        Args:
            existing_phases: List of already detected phases
            new_phases: List of newly detected phases
            
        Returns:
            Combined list of phases
        """
        if not new_phases:
        ...
        # Merge the lists
        all_phases = existing_phases.copy()
        
        # Add new phases, skipping any that overlap significantly with existing ones
        for new_phase in new_phases:
            overlaps_existing = False
            for existing_phase in existing_phases:
                # Check for overlap
    ...
    def _select_best_phases(self, phases):
        """
        Select the best phases to match the target count.
        ...
        # First, prioritize phases by quality (if available)
        quality_scores = {
            "high": 4,
            "medium": 3,
            "low": 2,
        ...
        # Sort phases first by quality, then by peak acceleration
        def phase_sort_key(phase):
            quality = phase.get("quality", None)
            quality_score = quality_scores.get(quality, 0)
            
            # For synthetic phases, use a slightly different approach
            if phase.get("isSynthetic", False) or phase.get("isInferred", False):
                return (quality_score, 0)
            
            return (quality_score, phase["peakAcceleration"])
        
        # Sort in descending order (highest quality and highest acceleration first)
        sorted_phases = sorted(phases, key=phase_sort_key, reverse=True)
        
        # Take the top N phases
        selected_phases = sorted_phases[:self.target_count]
        
        # Sort by time for the final result
        selected_phases.sort(key=lambda x: x["startTime"])
        
        return selected_phases
    
    def _assign_quality_scores(self, phases):
        """
        Assign quality scores to phases if they don't already have them.
        
        Args:
            phases: List of detected phases
            
        Returns:
            List of phases with quality scores
        """
        for phase in phases:
            if "quality" not in phase:
                # Determine quality based on other attributes
                if phase.get("isSynthetic", False):
                    phase["quality"] = "synthetic"
                elif phase.get("isInferred", False) or phase.get("isUnvalidated", False):
                    phase["quality"] = "low"
                elif "relaxation_step" in phase:
                    # Quality depends on which relaxation step was used
                    if phase["relaxation_step"] in ["min_duration_reduced", "peak_threshold_reduced"]:
                        phase["quality"] = "medium"
                    else:
                        phase["quality"] = "low"
                elif "reanalysis_method" in phase:
                    phase["quality"] = "medium"
                else:
                    # Original detection, should be high quality
                    phase["quality"] = "high"
...
class EnhancedDeadliftDetector:
        ...
        """
        def __init__(self, target_rep_count=None, verbose=False, visualize=True, 
                     force_orientation=None, force_target_count=False):
            """
            ...
            Args:
                ...
                force_target_count: Whether to force detection of exactly target_rep_count repetitions
            """
        ...
        def detect_phases(self, data):
            ...
            # Step 11: If force_target_count is enabled and we still don't have enough repetitions
            if self.force_target_count and self.target_rep_count is not None:
                if len(concentric_phases) != self.target_rep_count:
                    forced_processor = ForcedRepCountProcessor(self, self.target_rep_count, self.verbose)
                    concentric_phases = forced_processor.ensure_target_rep_count(concentric_phases)
                    self.phases = concentric_phases
                    
                    if self.verbose:
                        print(f"Forced phase count to {len(concentric_phases)} (target count: {self.target_rep_count})")
        ...
        def _export_comprehensive_csv(self, filename):
            ...
            for i, phase in enumerate(sorted_phases):
                ...
                if "quality" in phase:
                    export_data.loc[start_idx:end_idx, "phase_quality"] = phase["quality"]
        ...
        def _export_phase_summary_json(self, filename):
            ...
            try:
                ...
                metadata = {
                    ...
                    "force_target_count": bool(self.force_target_count),
                    "target_rep_count": int(self.target_rep_count) if self.target_rep_count is not None else None
                }
                ...
                for i, phase in enumerate(sorted_phases):
                    ...
                    if "quality" in phase:
                        rep_data["quality"] = str(phase["quality"])
                    
                    # Add detection method information
                    if phase.get("isFallback", False):
                        rep_data["detection_method"] = "fallback_detection"
                    elif phase.get("isSynthetic", False):
                        rep_data["detection_method"] = "synthetic"
                        if "synthMethod" in phase:
                            rep_data["synthetic_method"] = str(phase["synthMethod"])
                    elif phase.get("isInferred", False):
                        rep_data["detection_method"] = "inference"
                        if "inferenceMethod" in phase:
                            rep_data["inference_method"] = str(phase["inferenceMethod"])
                    elif "relaxation_step" in phase:
                        rep_data["detection_method"] = "relaxed_parameters"
                        rep_data["relaxation_step"] = str(phase["relaxation_step"])
                    elif "reanalysis_method" in phase:
                        rep_data["detection_method"] = "signal_reanalysis"
                        rep_data["reanalysis_method"] = str(phase["reanalysis_method"])
                    else:
                        rep_data["detection_method"] = "standard"
        ...
        def _select_best_phases(self, phases, target_count):
            ...
            for phase in sorted_phases:
                # Start with peak acceleration as the base score
                peak_score = phase["peakAcceleration"]
                
                # Apply quality multiplier if available
                if "quality" in phase:
                    quality = phase["quality"]
                    quality_multiplier = quality_scores.get(quality, 50) / 100.0
                    peak_score *= quality_multiplier
                
                # Default score is peak score
                phase["score"] = peak_score
                ...
                if len(intervals) >= 2:
                    ...
                    if std_interval < 0.5 * mean_interval:
                        ...
                        for i, phase in enumerate(sorted_phases):
                            ...
                            phase["score"] = phase["score"] * (1.0 - deviation_penalty * 0.5)
        ...
        def _visualize_detection(self, processed_data, peaks, phases):
            ...
            improved orientation awareness and quality indicators.
            ...
            """
            try:
                # Set figure size based on number of phases (larger for more phases)
                fig_height = 16 if len(phases) > 4 else 15
                ...
                # Plot 3: Detected Concentric Phases with Quality Indicators
                plt.subplot(5, 1, 3)
                plt.plot(processed_data["time_s"], processed_data["smoothed"], 'r-', label='Processed Signal')
                
                # Create color map for quality levels
                quality_colors = {
                    "high": "green",
                    "medium": "blue",
                    "low": "orange",
                ...
                # Define markers for detection methods
                method_markers = {
                    "standard": "s",         # square
                    "relaxed_parameters": "^", # triangle
                    "signal_reanalysis": "d", # diamond
                    "inference": "p",        # pentagon
                    "synthetic": "*"         # star
                }
                
                # Use a separate plotting loop for each quality level to build legend properly
                quality_plotted = set()
                method_plotted = set()
                
                # First plot all phase regions
                for i, phase in enumerate(phases):
                    ...
                    # Highlight the phase region with appropriate color
                    plt.axvspan(start_time, end_time, alpha=0.2, color=color)
                
                # Then plot start, peak, and end markers
                for i, phase in enumerate(phases):
                    quality = phase.get("quality", None)
                    color = quality_colors.get(quality, "gray")
                    ...
                    if phase.get("isSynthetic", False):
                        ...
                        method = "relaxed_parameters"
                    elif "reanalysis_method" in phase:
                        method = "signal_reanalysis"
                    else:
                        method = "standard"
                    
                    marker = method_markers.get(method, "o")
                    
                    # Mark start, peak, and end with method-specific marker
                    plt.plot(start_time, processed_data["smoothed"].iloc[phase["startIdx"]], 
                             marker=marker, color=color, markersize=6,
                             label=f"{method.replace('_', ' ').title()}" if method not in method_plotted else "")
                    
                    if method not in method_plotted:
                        method_plotted.add(method)
                    
                    plt.plot(peak_time, phase["peakAcceleration"], 
                             marker=marker, color=color, markersize=8)
                    
                    plt.plot(end_time, processed_data["smoothed"].iloc[phase["endIdx"]], 
                             marker=marker, color=color, markersize=6)
                    
                    # Add rep number and quality indicator
                    quality_label = f"({quality})" if quality else ""
                    plt.text(start_time, phase["peakAcceleration"], f" Rep {i+1} {quality_label}", 
                             fontsize=9, verticalalignment='bottom', color=color)
                ...
                for phase in phases:
                    quality = phase.get("quality", None)
                    color = quality_colors.get(quality, "gray")
                    plt.axvspan(phase["startTime"], phase["endTime"], alpha=0.2, color=color)
                
                plt.title('Rate of Change in Acceleration')
                plt.ylabel('Acceleration Derivative (g/s)')
                plt.grid(True)
                
                # Plot 5: Detection Method and Quality Summary
                plt.subplot(5, 1, 5)
                
                # Create a scatter plot showing quality and detection method
                quality_values = {"high": 4, "medium": 3, "low": 2, "synthetic": 1, None: 0}
                
                # Get values for plotting
                rep_numbers = list(range(1, len(phases) + 1))
                qualities = [quality_values.get(phase.get("quality", None), 0) for phase in phases]
                
                # Create detection method categories
                method_categories = []
                for phase in phases:
                    if phase.get("isSynthetic", False):
                        method_categories.append("synthetic")
                    elif phase.get("isInferred", False):
                        method_categories.append("inference")
                    elif "relaxation_step" in phase:
                        method_categories.append("relaxed_parameters")
                    elif "reanalysis_method" in phase:
                        method_categories.append("signal_reanalysis")
                    else:
                        method_categories.append("standard")
                
                # Create a categorical y-axis for methods
                method_y_positions = {"standard": 1, "relaxed_parameters": 2, "signal_reanalysis": 3, 
                                     "inference": 4, "synthetic": 5}
                
                y_positions = [method_y_positions.get(method, 6) for method in method_categories]
                
                # Plot rep quality scores
                plt.subplot(5, 1, 5)
                plt.grid(True, alpha=0.3)
                
                # Plot horizontal lines for method categories
                for method, y_pos in method_y_positions.items():
                    plt.axhline(y=y_pos, color='gray', linestyle='--', alpha=0.5)
                    plt.text(0, y_pos + 0.1, method.replace('_', ' ').title(), fontsize=9, 
                            verticalalignment='bottom')
                
                # Plot each repetition as a marker
                for i, (rep_num, y_pos, quality, method) in enumerate(zip(rep_numbers, y_positions, qualities, method_categories)):
                    color = list(quality_colors.keys())[list(quality_values.values()).index(quality)]
                    marker = method_markers.get(method, "o")
                    plt.scatter(rep_num, y_pos, c=quality_colors.get(color, "gray"), 
                               marker=marker, s=100, alpha=0.7)
                    
                    # Add rep number
                    plt.text(rep_num, y_pos - 0.2, f"{rep_num}", fontsize=9, 
                            horizontalalignment='center')
                
                # Add color legend for quality levels
                for i, (quality, color) in enumerate(quality_colors.items()):
                    if quality is not None:  # Skip None quality
                        plt.scatter([], [], c=color, label=f"{quality.title()} Quality", alpha=0.7)
                
                plt.title('Repetition Detection Method and Quality Summary')
...
def main(file_path, target_rep_count=None, visualize=True, verbose=False,
    output_dir=None, export_format='none', force_orientation=None, force_target_count=False):
        """
        ...
        Args:
            ...
            force_orientation: Optional dict to force specific orientation settings
            force_target_count: Whether to force the detection of exactly target_rep_count repetitions
        ...
        for i, phase in enumerate(concentric_phases):
            quality_str = f" ({phase['quality']})" if 'quality' in phase else ""
            print(f"Rep {i+1}{quality_str}:")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--force-target-count', action='store_true',
                           help='Force the output to contain exactly the number of repetitions specified in target_rep_count')
    ...
    main(
            ...
            force_orientation=force_orientation,
            force_target_count=args.force_target_count
        )